...
public class DiagramCtrl implements Initializable {
    @FXML
    ...
    @FXML
    private void setDrawBtn (ActionEvent event ) throws IOException, SQLException {

        Cube cube = cubeChoiceBox.getValue();
        if(cube != null){
            this.drawDiagram(cube.getId());
        }else{
            System.out.println("You have to choose a cube");
        }
    ...
    public void drawDiagram(int cubeId) throws SQLException, IOException {
        ...
        DiagramGrid dg = new DiagramGrid(cubeId);
        ...
        ClassTable fait = tDao.getFaitTables(tabsByLevel.get(1));

        ((ClassTableCtrl) fxmlLoader2.getController()).drawTable(fait);
        int mid = dg.size/2;
        ...
        for(int level=2; level<= tabsByLevel.size(); level++){
            ...
            for(ClassTable tab:currentLevel){
                ...
                ((ClassTableCtrl) fxmlLoader.getController()).drawTable(tab);
                int row = tab.row;
                int col = tab.col;
